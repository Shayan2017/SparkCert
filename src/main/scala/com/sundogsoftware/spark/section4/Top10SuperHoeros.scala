...
object Top10SuperHoeros {
  ...
  def main(args: Array[String]) {
    ...
    val sc = new SparkContext("local[*]", "Top10SuperHoeros")
    ...
    val top10Popular = flipped.takeOrdered(10)(Ordering[Int].reverse.on(x => x._1))

    for (heroId <- top10Popular) {
      // Look up the name (lookup returns an array of results, so we need to access the first result with (0)).
      val mostPopularName = namesRdd.lookup(heroId._2)(0)

      // Print out our answer!
      println(s"$mostPopularName is the most popular superhero with ${heroId._1} co-appearances.")
    }
